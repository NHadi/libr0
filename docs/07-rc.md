...
flowchart LR
    ...
    subgraph HEAP
        config["<b>Config</b><br/>db_url: String<br/>api_key: String"]
    end
    rc1 -->|"ptr"| config
    rc2 -->|"ptr"| config
    rc3 -->|"ptr"| config
    note["ALL three Rc pointers<br/>point to THIS SAME allocation!"]
```
...
flowchart LR
    ...
    subgraph HEAP
        c1["Config copy<br/><i>Separate allocation</i>"]
        c2["Config copy<br/><i>Another separate allocation</i>"]
        c3["Config copy<br/><i>Yet another allocation</i>"]
    end
    bp1 --> c1
    bp2 --> c2
    bp3 --> c3
...
flowchart LR
    ...
    subgraph "Heap (or Stack)"
        cfg["Config<br/><i>All references point to<br/>the same data (good!)</i>"]
    end
    r1 --> cfg
    r2 --> cfg
    r3 --> cfg
...
flowchart LR
    ...
    subgraph "After clone"
        ...
        subgraph "Heap "
            v2["1 | 2 | 3"]
            v3["1 | 2 | 3<br/><i>NEW copy!</i>"]
        end
    end
    d2 --> v2
    cl --> v3
    note2["Two separate allocations,<br/>all elements copied"]
```
...
flowchart LR
    subgraph "Before clone"
        ...
        subgraph "Heap"
            inner1["<b>count:</b> 1<br/>1 | 2 | 3"]
        end
    end
    r1 --> inner1
    subgraph "After Rc::clone"
        direction TB
        subgraph "Stack "
            r2["rc_data"]
            c2["clone2"]
        ...
        subgraph "Heap "
            inner2["<b>count:</b> 2<br/>1 | 2 | 3"]
        end
    end
    r2 -->|"ptr"| inner2
    c2 -->|"ptr"| inner2
    note3["Same allocation,<br/>counter incremented"]
```
...
flowchart LR
    ...
    subgraph Heap
        nodeA["<b>Rc&lt;Node&gt;</b><br/>strong_count: 2<br/>next: *"]
        nodeB["<b>Rc&lt;Node&gt;</b><br/>strong_count: 2<br/>next: *"]
    end
    a --> nodeA
    b --> nodeB
    nodeA -->|"next"| nodeB
...
flowchart LR
    subgraph Stack
        rc1w["rc1<br/>ptr *<br/><b>count:</b> 1"]
        rc2w["rc2<br/>ptr *<br/><b>count:</b> 1"]
    end
    subgraph Heap
        data1["data"]
    end
    rc1w --> data1
    rc2w --> data1
    wrong["Each Rc has its own count!<br/>They can't coordinate!<br/>❌ WRONG"]
```
...
flowchart LR
    ...
    subgraph Heap
        inner["<b>count:</b> 2<br/>data"]
    end
    rc1c --> inner
    rc2c --> inner
    correct["Both point to the SAME count<br/>✅ CORRECT"]
```
